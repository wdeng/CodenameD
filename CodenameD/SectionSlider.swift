...
@objc protocol SectionSliderDelegate {
    optional func sectionSliderShouldBeginTrack() -> Bool
...
class SectionSlider: UIControl {
    ...
    var currentValue: Double { // is in model value
        ...
        set {
            ...
            if !sectionSelectedByUser {
                delegate?.sectionSliderSectionDidChange?(currentSection, newVal: sectionForPosition(currentPosition))
    ...
    private func setupSlider() {
        ...
        let sectRect = CGRect(x: sectionFrames[0].minX+1, y: sectionFrames[0].minY, width: currentPosition, height: sectionFrames.first!.height)
        ...
        thumbLayer.position.x = currentPosition
    ...
    private var prevLocX: CGFloat = 0.0
    override func beginTrackingWithTouch(touch: UITouch, withEvent event: UIEvent?) -> Bool {
        if delegate?.sectionSliderShouldBeginTrack?() == false {
            return false
        }
    ...
    override func continueTrackingWithTouch(touch: UITouch, withEvent event: UIEvent?) -> Bool {
        let locx = min(max(touch.locationInView(self).x, bounds.minX), bounds.maxX)
        
        if thumbIsTracking {
            currentPosition = min(max(currentPosition + locx - prevLocX, bounds.minX), bounds.maxX)
            setCurrentSection(currentLocactionX: currentPosition)
            prevLocX = locx
